...
namespace Sovva.WebAPI.Services
{
    public class SupabaseStorageService : ISupabaseStorageService
    {
        ...
        public async Task<string?> GetSignedUrlAsync(string filePath, int expiresInSeconds = 3600)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                // ✅ Log warning but don't throw — file may have been deleted from storage
                _logger.LogWarning("Supabase sign failed for {FilePath} ({Status}): {Body}",
                    filePath, response.StatusCode, content);
                return null;
            }
